...
from src.config import WATCHLIST, WATCH_ONLY, SCHEDULE_DAY_OF_WEEK, SCHEDULE_HOUR, SCHEDULE_MINUTE
...
schedule_day = day_names.get(SCHEDULE_DAY_OF_WEEK, SCHEDULE_DAY_OF_WEEK)
st.sidebar.caption(f"Schedule: {schedule_day}s at {SCHEDULE_HOUR:02d}:{SCHEDULE_MINUTE:02d}")

# Check last run time from alerts
_recent_alerts = load_alerts(limit=5)
_last_run = next((a for a in _recent_alerts if a["alert_type"] in ("run_completed", "run_failed")), None)
if _last_run:
    st.sidebar.caption(f"Last run: {_last_run['created_at'][:16]}")
...
if view == "Portfolio Overview":
    ...
    if not alerts:
        st.info("No alerts. Alerts are generated by scheduled analysis runs.")
    else:
        alert_data = []
        for a in alerts:
            alert_data.append({
                "Time": a["created_at"][:16],
    ...
    if not earnings:
        st.info("No upcoming earnings data. Run: `python -m src.automation.scheduler --run-now`")
    else:
        from datetime import date as _date
        earnings_data = []
        for e in earnings:
            earnings_date = _date.fromisoformat(e["earnings_date"])
            days_until = (earnings_date - _date.today()).days
            earnings_data.append({
                "Ticker": e["ticker"],
                "Earnings Date": e["earnings_date"],
                "Days Until": days_until,
                "Est. EPS": f"${e['estimate_eps']:.2f}" if e.get("estimate_eps") is not None else "—",
            })
